#!/usr/bin/env python3
"""
patch_import_documents_bulk_20260730.py

一度きりの適用パッチ（使い切り）。
実行後は削除して構いません（再利用しない）。

内容:
  1. prisma/schema.prisma  — AuditAction enum に IMPORT_DOCUMENTS_RUN を追加
  2. prisma/migrations/20260730020000_add_import_documents_run_audit_action/migration.sql — 新規作成
  3. src/lib/audit.ts      — AuditAction union型に IMPORT_DOCUMENTS_RUN を追加
  4. src/lib/api-helpers.ts — ImportKeyInfo に createdBy を追加、withImportKey が渡すように変更
  5. src/app/api/projects/[id]/import/documents/route.ts — 新規作成（bulk importエンドポイント本体）

実行方法:
    cd ~/projects/meridian
    python3 patch_import_documents_bulk_20260730.py

実行後:
    npx prisma generate
    npx prisma migrate dev   # 新規マイグレーションをDBに適用
    npm run build            # コンパイルエラー0を確認
    git add -A && git commit -m "feat(import): bulk import documents endpoint" && git push
"""
import sys
...
# ------------------------------------------------------------------
# 1. prisma/schema.prisma — AuditAction enum
# ------------------------------------------------------------------
replace_exact(
    "prisma/schema.prisma",
    '  IMPORT_KEY_CREATE\n  IMPORT_KEY_REVOKE\n  @@map("audit_action")',
    '  IMPORT_KEY_CREATE\n  IMPORT_KEY_REVOKE\n  IMPORT_DOCUMENTS_RUN\n  @@map("audit_action")',
)
...
# ------------------------------------------------------------------
create_new(
    "prisma/migrations/20260730020000_add_import_documents_run_audit_action/migration.sql",
    '-- AlterEnum\nALTER TYPE "audit_action" ADD VALUE \'IMPORT_DOCUMENTS_RUN\';\n',
)

# ------------------------------------------------------------------
# 3. src/lib/audit.ts — AuditAction union型
# ------------------------------------------------------------------
replace_exact(
    "src/lib/audit.ts",
...
# ------------------------------------------------------------------
# 4. src/lib/api-helpers.ts — ImportKeyInfo に createdBy を追加
# ------------------------------------------------------------------
replace_exact(
...
# ------------------------------------------------------------------
# 5. 新規 bulk import エンドポイント
# ------------------------------------------------------------------
IMPORT_DOCUMENTS_ROUTE = r'''// src/app/api/projects/[id]/import/documents/route.ts
//
