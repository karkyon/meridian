...
import { FileText, Paperclip, Info, Layers, FlaskConical, TrendingUp } from "lucide-react";
...
export default function ProjectTabNav({
  ...
  const isBusinessAnalysis = pathname.startsWith(`${base}/business-analysis`);
  ...
  // 画像77193のスタイル: 背景つきセグメント型
  // アクティブ: 白背景・青テキスト・下線
  // 非アクティブ: 透明背景・グレーテキスト
  const tabClass = (active: boolean) =>
    `flex items-center gap-1.5 px-4 py-2 text-xs font-medium whitespace-nowrap transition-all border-b-2 ${
      active
        ? "border-[#1D6FA4] text-[#1D6FA4] font-semibold bg-white"
        : "border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50"
    }`;

  return (
    <nav className="bg-slate-50 border-b border-slate-200 flex items-center overflow-x-auto">
      <div className="flex items-center px-3 gap-0 min-w-max w-full">
        {/* ドキュメント */}
        ...
        <Link
          ...
          className={`${tabClass(isGithub)} ${
            !hasRepo ? "opacity-40 cursor-default pointer-events-none" : ""
          }`}
        ...
        >
          ...
          GitHub
          {!hasRepo && (
            <span className="text-[9px] bg-slate-200 text-slate-400 px-1 py-0.5 rounded ml-0.5">未設定</span>
          )}
        </Link>
        ...
        {role === "admin" && (
          <Link href={`${base}/ai-progress`} className={tabClass(isAi)}>
            <span className="text-xs">🤖</span> AI進捗推定
          </Link>
        )}
        ...
        {/* 事業性分析（Admin のみ） */}
        {role === "admin" && (
          <Link href={`${base}/business-analysis`} className={tabClass(isBusinessAnalysis)}>
            <TrendingUp size={13} />
            事業性分析
          </Link>
        )}
