      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {files.length === 0 ? (
          ...
          <div className="space-y-2">
            {files.map((file: any) => (
            <div key={file.id} className="file-list-row group flex-col items-stretch gap-1 cursor-pointer" onClick={() => setActiveFile(file)}>
              {/* 上段：アイコン・ファイル名・バッジ・ホバーボタン */}
              <div className="flex items-center gap-2">
                ...
                {/* MD・HTML：編集アイコン、WORD・PDF：プレビューアイコン */}
                {(file.fileType === "md" || file.fileType === "markdown" || file.fileType === "html" || file.fileType === "htm" || file.isEditable) ? (
                  <button
                    onClick={(e) => { e.stopPropagation(); setActiveFile(file); }}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-[#1A3A5C] text-[#1A3A5C] bg-white hover:bg-[#1A3A5C] hover:text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                    title="編集"
                  >
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setActiveFile(file); }}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-slate-400 text-slate-500 bg-white hover:bg-slate-700 hover:text-white hover:border-slate-700 text-xs font-medium opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                    title="プレビュー"
                  >
                ...
                <a
                  ...
                  className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-[#1A3A5C] opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                  title="ダウンロード"
                >
